...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS todos (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        title TEXT NOT NULL, category_id INTEGER REFERENCES categories(id),
        ...
        created_at TEXT DEFAULT (datetime('now')))""")
    try:
        c.execute("ALTER TABLE todos ADD COLUMN urgency TEXT DEFAULT 'someday'")
    except Exception:
        pass
    count = c.execute("SELECT COUNT(*) FROM categories").fetchone()[0]
    if count == 0:
        defaults = [
            ("Niere","🏫","#6C8EBF"),("Misfits","💼","#82B366"),
            ("ウェブフリ","🎯","#F0A30A"),("Threads（自分）","📱","#E84393"),
            ("自動化","🤖","#9673A6"),("JIKUU","🌀","#23A6D5"),
            ("あいたん講座","💫","#FF6B9D"),("プライベート","🏠","#D85C5C"),
            ("その他","📋","#888888")]
...
def get_todos(category_id=None, completed=None, urgency=None):
    conn = get_conn()
    query = """SELECT t.*, c.name as category_name, c.emoji as category_emoji, c.color as category_color
    ...
    if completed is not None:
        query += " AND t.completed=?"; params.append(1 if completed else 0)
    if urgency is not None:
        query += " AND t.urgency=?"; params.append(urgency)
...
def create_todo(data):
    conn = get_conn()
    c = conn.cursor()
    c.execute("INSERT INTO todos (title,category_id,priority,due_date,tags,notes,urgency) VALUES (?,?,?,?,?,?,?)",
        (data["title"], data.get("category_id"), data.get("priority","medium"),
         data.get("due_date"), data.get("tags",""), data.get("notes",""), data.get("urgency","someday")))
    conn.commit()
    row = conn.execute("""SELECT t.*, c.name as category_name, c.emoji as category_emoji, c.color as category_color
        FROM todos t LEFT JOIN categories c ON t.category_id=c.id WHERE t.id=?""", (c.lastrowid,)).fetchone()
    conn.close()
    return dict(row)
...
def update_todo(todo_id, data):
    conn = get_conn()
    fields, values = [], []
    for k in ["title","category_id","priority","due_date","tags","notes","completed","urgency"]:
        if k in data:
            fields.append(f"{k}=?"); values.append(data[k])
    ...
    if fields:
        ...
        conn.commit()
    row = conn.execute("""SELECT t.*, c.name as category_name, c.emoji as category_emoji, c.color as category_color
        FROM todos t LEFT JOIN categories c ON t.category_id=c.id WHERE t.id=?""", (todo_id,)).fetchone()
    conn.close()
    return dict(row)
